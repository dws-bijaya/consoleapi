...
class Server_Smtp:
	...
	def send_via_smtplib(self, host, port, timeout, secure, starttls, verify, auth, msg):
		...
		try:
			...
			if secure != 'ssl':
				...
				server = smtplib.SMTP_SSL(host, port, timeout=timeout, context=context if not verify else None)
			...
			if svr_resp:
				if isinstance(svr_resp, dict):
					try:
						...
						response['errno'], response['errmsg'] = ERROR_CODES.new(errno, errmsg)
						return response
					except Exception as e:
						pass
				...
				console_out.insert(0, "Connected to smtp://{0}:{1}/?starttls={2}&ssl={3}".format(host, port, "2", "1" if secure == 'ssl' else '0'))
			...
			if debug == 2:
				try:
					...
					pass
			smtp_code = e.smtp_code if hasattr(e, 'smtp_code') and e.smtp_code is not None else (ERROR_CODES.CATCHAALL_1000_Error[0] + e.errno) if hasattr(e, 'errno') and e.errno is not None else ERROR_CODES.CATCHAALL_1000_Error[0]
			...
			response['errno'], response['errmsg'] = ERROR_CODES.new(smtp_code, smtp_error)
			return response
		except Exception as e:
			if debug == 2:
				try:
					...
					pass
			errno = ERROR_CODES.CATCHAALL_1000_Error[0] if not hasattr(e, 'errno') or e.errno is None else (ERROR_CODES.CATCHAALL_1000_Error[0] + e.errno)
			errmsg = e.message if hasattr(e, 'message') and e.message is not None else e.strerror if hasattr(e, 'strerror') and e.strerror else repr(e)
			response['errno'], response['errmsg'] = ERROR_CODES.new(errno, errmsg)
			if e == socket.timeout:
	...
	def send(self, response, libray, host, port, secure, starttls, verify, auth, from_addrs, to_addrs, bcc_addrs, cc_addrs, subject, body_html, body_text):
		...
		try:
			...
			msg['Subject'] = subject
			msg['From'] = Address(from_addrs['display_name'], from_addrs['username'], from_addrs['domain'])
			msg['To'] = (Address(to_adr['display_name'], to_adr['username'], to_adr['domain']) for to_adr in to_addrs)
			if cc_addrs:
				msg['Cc'] = (Address(cc_addr['display_name'], cc_addr['username'], cc_addr['domain']) for cc_addr in cc_addrs)
			if bcc_addrs:
				msg['Bcc'] = (Address(bcc_addr['display_name'], bcc_addr['username'], bcc_addr['domain']) for bcc_addr in bcc_addrs)
			#msg['Bcc'] = ""
			#exit([msg.as_string()])
		except Exception as e:
			#raise e
		...
		timeout = 10
		response = self.send_via_smtplib(host, port, timeout, secure, starttls, verify, auth, msg)
		#exit([response])
		return response
		...
		for emailid in emailids:
			...
			try:
				...
				if not headers['From'].addresses[0].username or not email_domain:
					return None
				emailaddresses.append({'display_name': display_name, 'username': headers['From'].addresses[0].username, 'domain': email_domain, 'address': '{0}@{1}'.format(headers['From'].addresses[0].username, email_domain)})
