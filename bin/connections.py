...
class _Bloom_Filter:
	...
	def test_case(self):
		...
		#print_redis3_conn[1].bloomfilter_create("test", 11, 0.001)
...
class SApiConnections:
	...
	def redis3_conn(self, db=0):
		...
		if not self._redis3_pool:
			self._redis3_pool = redis.ConnectionPool(host=self.settings['REDIS3_HOST'], port=self.settings['REDIS3_PORT'], db=db, password=self.settings['REDIS3_PASSWORD'])
		lsterr = {}
		for _ in range(2):
			try:
	...
	def rabitmq_conn(self):
		...
		import pika
		credentials = pika.PlainCredentials(self.settings['RABITMQ_SERVER_USER'], self.settings['RABITMQ_SERVER_PASSWORD'])
		lsterr = None
		for _ in range(5):
			try:
				connection = pika.BlockingConnection(
				    pika.ConnectionParameters(host=self.settings['RABITMQ_SERVER_HOST'],
				                              port=self.settings['RABITMQ_SERVER_PORT'],
				                              virtual_host=self.settings['RABITMQ_SERVER_VHOST'],
	...
	def rabitmq2_conn(self):
		...
		for _ in range(5):
			try:
				client = Client('{host}:{port}'.format(host=self.settings['MSGQUEUE_SERVER_HOST'], port=self.settings['MSGQUEUE_SERVER_API_PORT']),
				                self.settings['MSGQUEUE_SERVER_USER'],
				                self.settings['MSGQUEUE_SERVER_PASSWORD'],
				                timeout=self.settings['MSGQUEUE_SERVER_TIMEOUT'])
	...
	def ocl_nosqldb_conn(self):
		...
		socket.connect("tcp://%s:%s" % (self.settings['ORACLE_NOSQLDB_HOST'], self.settings['ORACLE_NOSQLDB_PORT']))
		socket.setsockopt(zmq.RCVTIMEO, self.settings['ORACLE_NOSQLDB_RCVTIMEO'])
	...
	def _hazelcast_conn(self):
		...
		config.logger_config.level = 100 if self.settings['LOG_ENABLED'] == False else (
		    self.settings['HAZELCAST_LOGGER_LEVELS'][self.settings['LOG_LEVEL']] if self.settings['LOG_LEVEL'] in self.settings['HAZELCAST_LOGGER_LEVELS'] else self.settings['HAZELCAST_LOGGER_LEVELS']['NOTSET'])
		[config.network_config.addresses.append(host) for host in self.settings['HAZELCAST_HOSTS']]
		lsterr = None
		for _ in range(2):
			try:
	...
	def _cassandra_conn(self, db=None):
		...
		db = db if db is not None else self.settings['CASSANDRA_DBS'][0]
		...
		for _ in range(2):
			try:
				cluster = Cluster(self.settings['CASSANDRA_HOSTS'], port=self.settings['CASSANDRA_PORT'])
	...
	def _localmysql_conn(self, db=None):
		db = db if db is not None else self.settings['LOCALMYSQL_DBS'][0]
		...
		for _ in range(2):
			try:
				self.localmysql_conn = mysql.connector.connect(user=self.settings['LOCALMYSQL_USER'],
				                                               password=self.settings['LOCALMYSQL_PASSWORD'],
				                                               host=self.settings['LOCALMYSQL_HOST'],
				                                               port=self.settings['LOCALMYSQL_PORT'],
				                                               database=self.settings['LOCALMYSQL_DBS'][0])
				break
			except Exception as e:
	...
	def _remotemysql_conn(self, db=None):
		db = db if db is not None else self.settings['REMOTEMYSQL_DBS'][0]
		...
		for _ in range(2):
			try:
				self.remotemysql_conn = mysql.connector.connect(user=self.settings['REMOTEMYSQL_USER'],
				                                                password=self.settings['REMOTEMYSQL_PASSWORD'],
				                                                host=self.settings['REMOTEMYSQL_HOST'],
				                                                port=self.settings['REMOTEMYSQL_PORT'],
				                                                database=self.settings['REMOTEMYSQL_DBS'][0])
				break
			except Exception as e:
				lsterr = {"errno": e.errno, "errmsg": str(e)}
				time.sleep(0.5)
	...
	def _report_conn(self, db):
		...
		for _ in range(2):
			try:
				self.report_conn = MongoClient("{host}:{port}".format(host=self.settings['MONGO_REPORT_HOST'], port=self.settings['MONGO_REPORT_PORT']))
				lsterr = {}
				break
			except Exception as e:
				lsterr = {"errno": e.errno if 'errno' in e else 1, "errmsg": str(e)}
	...
	def _nfs_conn(self, reset=False, close=False):
		...
		context = zmq.Context()
		socket = context.socket(zmq.REQ)
		socket.connect('tcp://{NFS_HOST}:{NFS_PORT}'.format(NFS_HOST=self.settings['NFS_HOST'], NFS_PORT=self.settings['NFS_PORT']))
	...
	def _redis_conn(self, db=0):
		...
		if not self.redis_pool:
			self.redis_pool = redis.ConnectionPool(host=REDIS_HOST, port=REDIS_PORT, db=db, decode_responses=True, password=REDIS_PASSWORD)
		lsterr = {}
		for _ in range(2):
			try:
	...
	def _redis2_conn(self, db=0):
		...
		if not self.redis2_pool:
			self.redis2_pool = redis.ConnectionPool(host=self.settings['REDIS2_HOST'], port=self.settings['REDIS2_PORT'], db=db, password=self.settings['REDIS2_PASSWORD'])
		lsterr = {}
		for _ in range(2):
			try:
	...
	def _data_conn(self, db):
		...
		for _ in range(2):
			try:
				self.data_conn = MongoClient("{host}:{port}".format(host=self.settings['MONGO_DATA_HOST'], port=self.settings['MONGO_DATA_PORT']))
				lsterr = {}
				break
			except Exception as e:
				lsterr = {"errno": e.errno if 'errno' in e else 1, "errmsg": str(e)}
	...
	def _mongo_conn(self, db):
		...
		for _ in range(2):
			try:
				self.mongo_conn = MongoClient("{host}:{port}".format(host=self.settings['MONGO_MASTER_HOST'], port=self.settings['MONGO_MASTER_PORT']))
				lsterr = {}
				break
			except Exception as e:
				lsterr = {"errno": e.errno if 'errno' in e else 1, "errmsg": str(e)}
